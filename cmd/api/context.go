...
// Define a custom contextKey type for the request context, with the underlying type
// string.
...
// Retrieves the User struct from the request context. The only time that we'll use this
// helper is when we logically expect there to be User struct value in the context, and
